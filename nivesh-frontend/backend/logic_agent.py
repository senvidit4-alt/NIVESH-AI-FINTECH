...
def calculate_var_cvar(symbol: str, confidence: float = 0.95) -> dict:
    try:
        hist = yf.Ticker(symbol, session=YF_SESSION).history(period="1y")
...
def get_sector_breakdown(symbols: list) -> dict:
    ...
    for sym in symbols:
        try:
            sector = yf.Ticker(sym, session=YF_SESSION).info.get("sector", "Unknown")
        except Exception:
            sector = "Unknown"
...
def analyze_stock_trend(symbol: str) -> str:
    ...
    data = yf.Ticker(symbol, session=YF_SESSION).history(period="1mo")
...
def technical_analysis(symbol: str) -> str:
    ...
    data = yf.Ticker(symbol, session=YF_SESSION).history(period="3mo")
...
def compare_stocks(symbol1: str, symbol2: str) -> str:
    ...
    s1 = yf.download(symbol1, period="1mo", session=YF_SESSION)['Close']
    s2 = yf.download(symbol2, period="1mo", session=YF_SESSION)['Close']
...
def data_fetch_node(state):
    ...
    try:
        hist = yf.Ticker(symbol, session=YF_SESSION).history(period="3mo")
